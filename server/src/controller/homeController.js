    ...
    for (var i = 0; i < data.dataToSend2.updatedPlayers.length; i++) {
      ...
      var result = await pool // var hay const
        ...
        .input(
          "varHoTen",
          sql.NVarChar(MAX),
          data.dataToSend2.updatedPlayers[i].TenCauThu
        )
        .input(
          "varLoai",
          sql.NVarChar(MAX),
          data.dataToSend2.updatedPlayers[i].LoaiCauThu
        )
        .input("varNgaySinh", sql.Date, date)
        .input(
          "varGhiChu",
          sql.NVarChar(MAX),
          data.dataToSend2.updatedPlayers[i].GhiChu
        )
        .input(
          "varId",
          sql.VarChar(MAX),
          data.dataToSend2.updatedPlayers[i].MaCauThu
        )
