...
ALTER TRIGGER TRIGGER_LICHLAMVIEC 
ON LICHLAMVIEC
FOR INSERT
AS
    BEGIN
    ...
    --     IF EXISTS (SELECT * FROM INSERTED i JOIN LICHLAMVIEC LLV ON i.NGAY = LLV.NGAY WHERE i.GIOBATDAU < LLV.GIOKETTHUC OR LLV.GIOBATDAU < i.GIOKETTHUC AND I.MANHASI = LLV.MANHASI AND I.MALICH != LLV.MALICH)
    --   BEGIN
...
AS
    ...
    IF EXISTS (SELECT * FROM inserted I INNER JOIN LICHHEN LH ON (I.MABENHNHAN = LH.MABENHNHAN OR I.NHASI = LH.NHASI) AND ((DATEDIFF(DAY, I.THOIGIAN, LH.GIOKETTHUC) > 0) OR (DATEDIFF(DAY, LH.THOIGIAN, I.GIOKETTHUC) < 0)) AND I.NGAYHENYEUCAU = LH.NGAYHENYEUCAU AND I.MALICHHEN = LH.MALICHHEN)
	    BEGIN
            RAISERROR('LICH HEN KHONG DUOC TRUNG VOI LICH CUA BENH NHAN HOAC NHA SI', 16, 1)
            ROLLBACK TRANSACTION
        END
    ELSE IF EXISTS (
    SELECT 1
    FROM inserted I
    INNER JOIN LICHLAMVIEC LLV ON I.NHASI = LLV.MANHASI
    WHERE (
            DATEDIFF(MINUTE, LLV.GIOBATDAU, I.THOIGIAN) < 0 OR
            DATEDIFF(MINUTE, LLV.GIOKETTHUC, I.GIOKETTHUC) > 0 OR
            DATEDIFF(MINUTE, LLV.GIOBATDAU, I.GIOKETTHUC) < 0 OR
            DATEDIFF(MINUTE, LLV.GIOKETTHUC, I.THOIGIAN) > 0 OR
            DATEDIFF(DAY, I.NGAYHENYEUCAU, LLV.NGAY) != 0 
    )
    )
    BEGIN
        RAISERROR('NHA SI KHONG LAM VIEC TRONG THOI GIAN NAY', 16, 1)
        ROLLBACK TRANSACTION
    END
    ELSE 
...
-- values (1, NULL, 'Nam', NULL, NULL, 'KHOE', 'KHONG KHOE')
